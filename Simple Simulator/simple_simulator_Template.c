...
// NOVA FUNÇÃO : calcula a média aritmética entre dois registardores
#define MED 38      // "100110"; -- MED Rx Ry Rz                	-- Rx <- (Ry + Rz) / 2  Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
...
//#include <fcntl.h>        // Fileopen - Fileclose - fprintf - fscanf -- COMENTADO PARA WINDOWS
#include <math.h>
...
// Rotacao (Adicionado pois estava no header conio.h do Linux mas nao no Windows MinGW padrao as vezes)
...
loop:
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				case INCHAR:
					...
					if(_kbhit())
						TECLADO = _getch(); // alterado: _getch() não precisa de enter
					else
						TECLADO = 255;
				...
				// IMPLEMENTAÇÃO DA MED (OPCODE 38)
				case MED:
					selM3 = rx; 
                    selM4 = ry; 
                    OP = opcode; 
				...
				case BREAKP: 
					key = _getch(); // Alterado para Windows (conio.h)
					// -----------------------------
					state=STATE_FETCH;
					break;
		...
		case STATE_HALTED:
			printf("\n");
			key = _getch(); // Alterado para Windows
			if (key == 'r') goto inicio; 
			if (key == 'q') goto fim; // Corrigido identacao
			break;
	...
	switch(pega_pedaco(OP, 5, 4)) {
		case ARITH:
			switch(OP) {
				...
				// IMPLEMENTAÇÃO NA ULA ---
				case MED:
				{
					// Calcula a soma de x e y
    				unsigned long soma_temp = (unsigned long)x + y;
    				...
    				if (result > MAX_VAL) {
        				...
        				auxFRbits[ARITHMETIC_OVERFLOW] = 0; // Limpa flags (mantendo o comportamento original)
    				}
					auxFRbits[CARRY] = 0; // Limpa carry
					break;
				}
				// -----------------------------------
